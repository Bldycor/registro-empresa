...
- **Menús agrupados por tarea y plegables** (`roleNav` en `src/components/panel-sidebar.tsx` y `TABS` en `evidencia-ep-nav.tsx`). El sidebar muestra los grupos cerrados y abre solo el de la página actual; el usuario puede abrir o cerrar los demás mientras dure la visita (no se guarda en el navegador). Cada opción lleva su ícono, el resumen como `title` y, cuando está activa, ese mismo resumen impreso debajo.
