...
export function FormalizacionesPanel() {
  const [items, setItems] = useState<Formalizacion[] | null>(null);
  ...
  function load() {
    fetch("/api/instructor/formalizaciones")
      .then((res) => res.json())
      .then((data) => setItems(data.formalizaciones ?? []));
  }
  ...
  async function avalar(id: string, estado: "APROBADA" | "RECHAZADA") {
    setBusy(id);
    await fetch(`/api/instructor/formalizaciones/${id}`, {
      method: "PATCH",
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-sm text-zinc-700 dark:text-zinc-300">{s.tipoDocumento}</p>
